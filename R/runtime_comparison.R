...
for( j in 1:length(model_size)){
  for(i in c(1:k)){ 
    ...
    if (run[3]){
      ...
      df_times$rmse[3] <- sqrt(mean(unlist(predict(fit, data[501:100,]) - data$Y[501:1000])^2))
    }
    ...
    print(j)
    df<- rbind(df,df_times[complete.cases(df_times),])
    print(df, max = 100)
  }
